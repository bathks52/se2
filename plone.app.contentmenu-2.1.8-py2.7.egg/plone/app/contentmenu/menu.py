...
from plone.app.contentmenu.interfaces import IAccessibilityCheckerMenu
from plone.app.contentmenu.interfaces import IAccessibilityCheckerSubMenuItem
...
@implementer(IAccessibilityCheckerSubMenuItem)
class AccessibilityCheckerSubMenuItem(BrowserSubMenuItem):

    MANAGE_SETTINGS_PERMISSION = 'Accessibility: Check accessibility'

    title = _(u'check_accessibility_link', default=u'Accessibility check')
    submenuId = 'plone_contentmenu_accessibilitychecker'
    order = 75
    ...
    @property
    def extra(self):
        return {'id': 'plone-contentmenu-accessbilitychecker',
                'li_class': 'plonetoolbar-accessibility-checker'}

    @property
    def description(self):
        if self._manageSettings():
            return _(
                u'title_check_accessibility',
                default=u'Check the accessibility compliance'
            )
        else:
            return u''
...
@implementer(IAccessibilityCheckerMenu)
class AccessibilityCheckerMenu(BrowserMenu):
